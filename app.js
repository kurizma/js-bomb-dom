...
for (let r = 0; r < numRows; r++) {
    ...
    for (let c = 0; c < numCols; c++ ) {
        if ( r === 0 || c === 0 || r === numRows -1 || c === numCols-1 || (r % 2 === 0 && c % 2 === 0) ) {
            level[r][c] = 1; // wall
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.key === ' ') {
        if (!bombs.some(b => b.row === player.row && b.col === player.col)) {
            const bombPos = { row: player.row, col: player.col };
            bombs.push(bombPos);
            renderGrid();
            setTimeout(() => {
                // remove bomb and show explosion
                bombs = bombs.filter(b => !(b.row === bombPos.row && b.col === bombPos.col));
                showExplosion(bombPos.row, bombPos.col);
            }, 2000);
        }
    }
});
